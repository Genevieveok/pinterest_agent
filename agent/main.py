...
IMAGE_HOST_BRANCH = os.getenv('IMAGE_HOST_BRANCH') or CONFIG.get('image_host_branch') or 'gh-pages'
...
def safe_run_with_retries(func, attempts=3, delay=5, *args, **kwargs):
    ...
    logger.error('All %s attempts failed for %s', attempts, getattr(func,'__name__',str(func)))
    return None
...
def run_repins():
    ...
    for idx, bk in enumerate(selected_boards):
        cfg = BOARDS[bk]
        picked = safe_run_with_retries(repin_for_board, attempts=3, delay=3, board_key=bk, board_cfg=cfg, quota=1, filters=filters, sleep_fn=lambda: short_random_sleep(2,6))
        if picked:
            all_picked.extend(picked)
        human_sleep_between_pins(idx, total_pins=repins_needed+CONFIG['daily_pins']['new_pins'])
    return all_picked
...
def run_new_pins():
    ...
    for idx, p in enumerate(posts):
        ...
        local_img = None
        if use_ai:
            local_img = safe_run_with_retries(build_aesthetic_image, attempts=2, delay=5, background_url=meta.get('image'), title_text=title_for_image, hf_token=HF_TOKEN)
        if not local_img:
            local_img = safe_run_with_retries(build_aesthetic_image, attempts=1, delay=1, background_url=meta.get('image'), title_text=title_for_image, hf_token=None)
        if not local_img:
            continue
        public_url = None
        try:
            public_url = upload_image_to_github(local_img, repo=GITHUB_REPO, branch=IMAGE_HOST_BRANCH, token=os.getenv('GITHUB_TOKEN'))
        except Exception as e:
            logger.warning('upload_image_to_github failed, falling back to meta image: %s', e)
        ...
        res = safe_run_with_retries(save_pin_to_board, attempts=2, delay=3, board_id=matched_board['id'], image_url=public_url, title=meta.get('title'), description=meta.get('description'), link=meta.get('url'))
        if not res:
            continue
        pin_id = res.get('id')
        if pin_id:
            cur.execute('INSERT OR IGNORE INTO blog_pins (post_url, pinterest_pin_id) VALUES (?, ?)', (p, pin_id))
            conn.commit()
            created_new.append(pin_id)
        human_sleep_between_pins(idx + CONFIG['daily_pins']['repins'], total_pins=CONFIG['daily_pins']['repins']+CONFIG['daily_pins']['new_pins'])
    conn.close()
    return created_new
